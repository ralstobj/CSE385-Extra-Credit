...
namespace WebAPI {
	...
	public class api : System.Web.Services.WebService {
        ...
        [WebMethod]
        public void GetCoursesBySubject(string subject)
        {
            try {
        ...
        [WebMethod]
        public void GetCoursesByCRN(string crn)
        {
            //http://ws.miamioh.edu/courseSectionV2/201920.json?crn=82261
            try
            {
                using (var client = new WebClient())
                {
                    var json = client.DownloadString("http://ws.miamioh.edu/courseSectionV2/201920.json?crn=" + crn);
                    dynamic dynJson = JsonConvert.DeserializeObject(json);
                    streamJson(dynJson.ToString());
                }
            }
